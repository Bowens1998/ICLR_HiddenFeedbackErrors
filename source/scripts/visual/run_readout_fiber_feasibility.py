...
def main():
    ...
    a=p.parse_args();out=Path(a.output);out.mkdir(parents=True,exist_ok=False);records=[];bindings=[];started=time.monotonic()
    for g in range(6):
        ...
        for slot in (2,3,4):
            ...
            print('MODEL',row['model_index'],'valid',sum(x['constrained']['valid'] for x in records[-40:]),'/40',flush=True)
    assert len(records)==720
    result=dict(status='ENGINEERING_FEASIBILITY_COMPLETE_NOT_ROLLOUT_EVIDENCE',records=records,bindings=bindings,protocol_sha256=sha(a.protocol),source_sha256=sha(__file__),projection_source_sha256=sha(Path(__file__).with_name('readout_fiber.py')),elapsed_seconds=time.monotonic()-started)
