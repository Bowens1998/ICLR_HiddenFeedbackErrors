...
import numpy as np
import torch
from factorial_model import ARMS,make_model
from train_visual_factorial import tensor_hash

def main():
    p=argparse.ArgumentParser()
    for key in ['run','official','config','data']:p.add_argument('--'+key,required=True)
    p.add_argument('--epochs',type=int,default=100);a=p.parse_args();torch.set_num_threads(2);run=Path(a.run);rows=[]
    ...
    for arm in ARMS:
        ...
        assert len(r['epochs'])==a.epochs==r['expected_epochs'];assert [v['epoch'] for v in r['epochs']]==list(range(1,a.epochs+1))
        assert r['best_epoch']==1+int(np.argmin([v['validation']['loss'] for v in r['epochs']]))
        ...
        for key,module in [('encoder',model.encoder),('action_encoder',model.action_encoder),('temporal_core',model.predictor.core if arm.endswith('state') else model.predictor)]:assert tensor_hash(module)==r['initial_hashes'][key]
        ...
        with torch.inference_mode():
            ...
            assert native.shape==(2,8,6 if arm.endswith('state') else 192)
    ...
    result={'accepted_arms':ARMS,'epochs':a.epochs,'shared_initializations':'encoder, action encoder; temporal core within architecture verified',
