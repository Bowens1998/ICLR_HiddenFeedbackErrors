...
def main():
    ...
    assert r['status']=='SHUFFLED_FIBER_ROLLOUT_REQUIRES_NUMPY_ACCEPTANCE' and r['plan_sha256']==sha(a.plan) and r['protocol_sha256']==sha(a.protocol) and r['source_sha256']==sha(Path(__file__).with_name('run_shuffled_fiber_rollout.py'))
    assert r['anchor_checks']==1536 and r['readout_checks']==3072 and r['goal_checks']==384 and len(r['rows'])==3
    ...
    for k,slot in enumerate((2,3,4)):
        ...
        np.testing.assert_array_equal(z['fiber_tokens'][:,:,0],fz['constrained'].reshape(4,128,192));np.testing.assert_array_equal(z['reset_tokens'][:,:,0],z['observed_tokens'][:,:,0]);np.testing.assert_array_equal(z['shuffled_tokens'][:,:,0],fz['shuffled'].reshape(4,128,192))
        ...
        for path in ('free','fiber','shuffled','reset','observed'):
        ...
        err=max(float(np.max(abs(normalized[path][:,:,0]-normalized['free'][:,:,0]))) for path in ('fiber','shuffled'));assert err<=1e-6;maxerr=max(maxerr,err)
    ...
    result=dict(status='PASS_ALL3_SHUFFLED_FIBER_ROLLOUT_RECONSTRUCTIONS',index=g,rows=rows,seeds=common[1].tolist(),horizons=[5,10,15,20,25],max_initial_normalized_readout_error=maxerr,report_sha256=sha(d/'report.json'),source_sha256=sha(__file__),protocol_sha256=sha(a.protocol),scope='Independent NumPy decode/metrics and exact saved-array baseline, correction and reset identities. Full neural free trajectory anchored by extraction; modified neural trajectories are not independently reimplemented.')
