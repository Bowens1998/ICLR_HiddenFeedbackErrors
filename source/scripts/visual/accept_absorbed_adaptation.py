...
def main():
    ...
    assert r['config_sha256']==sha(a.config) and r['source_sha256']==sha(Path(__file__).with_name('fit_absorbed_adaptation.py'))
    ...
    h0=dict(np.load(frozen/stage/'initial.npz'));original_norm=dict(np.load(frozen/stage/'weights.npz'))
    w=h0['0.weight'].astype(float)/original_norm['scale'][None,:]
    b=h0['0.bias'].astype(float)-w@original_norm['mean']
    h0['0.weight']=w.astype(np.float32);h0['0.bias']=b.astype(np.float32)
    hd=max(float((v-torch.tensor(h0[k])).abs().max()) for k,v in checkpoint['head'].items())
    ...
    for k in ['target_mean','target_scale']:np.testing.assert_array_equal(checkpoint['normalization'][k],h[k])
    np.testing.assert_array_equal(checkpoint['normalization']['mean'],np.zeros(192))
    np.testing.assert_array_equal(checkpoint['normalization']['scale'],np.ones(192))
    ...
    head=nn.Sequential(nn.Linear(192,256),nn.ReLU(),nn.Linear(256,256),nn.ReLU(),nn.Linear(256,6)).double().cuda()
    ...
    result=dict(status='PASS',index=i,stage=stage,report_sha256=sha(run/'report.json'),verifier_sha256=sha(__file__),train_predictions_sha256=sha(run/'accepted_train_predictions.npz'),metrics={k:float(v.mean()) for k,v in metrics(predictions,poses['target']).items()},scope='Final weights/source/data bindings, identity input normalization and unchanged target scaling, encoder/head deltas independently checked against the appropriate absorbed initialization; all512 training images decoded with Torch float64 head and independent NumPy agreement. Does not replay2000 optimizer updates, independently implement encoder, or establish test performance.')
