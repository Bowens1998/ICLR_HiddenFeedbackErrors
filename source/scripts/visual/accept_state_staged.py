"""Staged direct-state arithmetic verification; original verifier unchanged."""
...
assert r['checkpoint']==entry['checkpoint'] and r['training_path']==entry['training_path']
...
assert r['score_space']=='state'
out=Path(a.output);out.mkdir(parents=True,exist_ok=False)
precision=configure_evaluation_precision()
gm=torch.tensor(mn,device='cuda',dtype=torch.float32);gs=torch.tensor(sd,device='cuda',dtype=torch.float32)
full64_diagnostics=[];bitwise_cases=0;gpu_max=0.
...
for row,item in zip(r['cases'],manifest['cases']):
 ...
 original_delta=np.abs(rebuilt-costs);violations=int((original_delta>atol+5e-5*np.abs(costs)).sum())
 if violations:full64_diagnostics.append(dict(case=i,violations=violations,max_difference=float(original_delta.max())))
 # Verify the executed FP32 affine map, then independently evaluate its physical cost in FP64.
 physical=z['tokens']*sd.astype(np.float32)+mn.astype(np.float32)
 physical_goal=z['goal_tokens']*sd.astype(np.float32)+mn.astype(np.float32)
 replay=[]
 with torch.inference_mode():
  pg=torch.tensor(z['goal_tokens'],device='cuda')*gs+gm
  np.testing.assert_array_equal(pg.cpu().numpy(),physical_goal)
  for t in range(30):
   ps=torch.tensor(z['tokens'][t],device='cuda')*gs+gm
   np.testing.assert_array_equal(ps.cpu().numpy(),physical[t])
   angle=torch.atan2(ps[:,4],ps[:,5])-torch.atan2(pg[4],pg[5]);angle=torch.atan2(angle.sin(),angle.cos())
   replay.append(((ps[:,2:4]-pg[2:4]).square().sum(1)+900*angle.square()).cpu().numpy())
 replay=np.stack(replay);np.testing.assert_allclose(replay,costs,rtol=5e-5,atol=atol)
 bitwise_cases+=int(np.array_equal(replay,costs));gpu_max=max(gpu_max,float(np.abs(replay-costs).max()))
 state=physical.astype(float);estimated_goal=physical_goal.astype(float)
 angle=np.arctan2(state[...,4],state[...,5])-np.arctan2(estimated_goal[4],estimated_goal[5]);angle=(angle+np.pi)%(2*np.pi)-np.pi
 rebuilt=np.sum((state[...,2:4]-estimated_goal[2:4])**2,-1)+900*angle**2
...
result.update(verification_policy='staged_fp32_direct_state_v1',original_summary_sha256=hashlib.sha256((run/'summary.json').read_bytes()).hexdigest(),precision=precision,full_float64_score_violations=full64_diagnostics,bitwise_score_cases=bitwise_cases,max_matched_float32_score_difference=gpu_max,scope='Original execution and acceptance retained; saved FP32 tokens and affine map verified on CPU/GPU, physical cost independently reconstructed in FP64; no scores or actions changed')
