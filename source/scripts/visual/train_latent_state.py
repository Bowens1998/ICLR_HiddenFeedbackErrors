...
from latent_state_model import ARMS,make_model,state_features,state_losses
...
def main():
    ...
    for path in [Path(__file__),Path(__file__).with_name('update_budget.py'),Path(__file__).with_name('latent_state_model.py'),Path(__file__).with_name('factorial_model.py'),Path(__file__).with_name('lewm_adapter.py'),Path(a.official)/'jepa.py',Path(a.official)/'module.py',Path(a.config)]:
    ...
    model=model.cuda();opt=torch.optim.AdamW(model.parameters(),lr=5e-5,weight_decay=1e-3)
    ...
    report={'arm':arm,'seed':a.seed,'evaluations':[],'gpu':torch.cuda.get_device_name(),'job_id':os.environ.get('SLURM_JOB_ID'),'parameters':sum(p.numel() for p in model.parameters()),
            ...
            'scope':'state-supervised control; 192-dimensional recursive tokens with a separate shared observation/prediction state head; extra labels and extra head parameters; no latent prediction or SIGReg loss',
        ...
        for split,loader in loaders.items():
            ...
            for step,(pixels,actions,states) in enumerate(batches):
                ...
                with torch.set_grad_enabled(training),torch.autocast('cuda',dtype=torch.bfloat16):
                    ...
                    pred_loss,obs_loss=state_losses(model,pred,z['emb'],targets)
                    reg_loss=pred_loss.new_zeros(());loss=.5*(pred_loss+obs_loss)
