...
def main():
    ...
    a=p.parse_args();plan=json.loads(Path(a.plan).read_text());data={};bindings=[];seeds=None;conditions=['unit_latent','unit_decoded_teacher','unit_physical_labels'];paths=['free','fiber','shuffled','reset','observed']
    for g in range(6):
        d=Path(a.runs)/f'job_{g}';r=json.loads((d/'report.json').read_text());ac=json.loads((d/'acceptance.json').read_text())
        assert ac['status']=='PASS_ALL3_SHUFFLED_FIBER_ROLLOUT_RECONSTRUCTIONS' and ac['source_sha256']==sha(Path(__file__).with_name('accept_shuffled_fiber_rollout.py')) and ac['report_sha256']==sha(d/'report.json')
        ...
        assert r['anchor_checks']==1536 and r['readout_checks']==3072 and r['goal_checks']==384 and ac['max_initial_normalized_readout_error']<=1e-6
    ...
    draws=np.random.default_rng(1366001).integers(0,128,(10000,128));groups=[];effects=[]
    ...
    for ref,label in enumerate(streams):
        for c,condition in enumerate(conditions):
            ...
            for path in ['fiber','shuffled','reset']:
    ...
    result=dict(status='COMPLETE18_MODELS_GUIDED_VERSUS_SHUFFLED_INTERVENTION',groups=groups,contrasts=effects,guidance_contrasts=guidance_contrasts,bindings=bindings,seeds=seeds,horizons=[5,10,15,20,25],complete_free_trajectory_anchors=9216,readout_preservation_checks=18432,plan_sha256=sha(a.plan),protocol_sha256=sha(a.protocol),source_sha256=sha(__file__),statistics_source_sha256=sha(Path(__file__).with_name('summarize_task_coordinate_horizon.py')),bootstrap=dict(seed=1366001,replicates=10000,scope='Retrospective secondary95% marginal conditional shared-goal intervals;128 goals, conditional on one fixed donor permutation and fixed models; streams/horizons are not replicates.',designated_mechanism_endpoint='25-action block-position MSE; earlier horizons describe propagation'),scope='Trajectory-matched versus shuffled observed guidance at a common displacement after first five actions. Fiber preserves immediate task readout; full reset also changes it. Neither intervention is a deployable policy.')
