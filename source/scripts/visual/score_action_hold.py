"""Frozen state-model paired action-hold diagnostic."""
...
def main():
    ...
    with torch.inference_mode():
        for item in manifest['cases']:
            ...
            np.savez_compressed(out/f"case_{item['index']:03d}_predictions.npz",predicted_tokens=predicted,predicted_future_tokens=native[:,3:].cpu().numpy(),realized_tokens=realized,goal_tokens=goal,
