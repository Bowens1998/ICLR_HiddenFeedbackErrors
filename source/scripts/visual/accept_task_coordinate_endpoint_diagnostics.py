...
def main():
    ...
    assert r['source_sha256']==sha(Path(__file__).with_name('extract_task_coordinate_endpoint_diagnostics.py'))
    route_indices=[i for i,route in enumerate(plan['routes']) if plan['models'][route['model_index']]['original_model_index']==r['index']]
    assert len(route_indices)==(10 if r['index']%2==0 else 6)
    assert r['same_image_checks']==256*len(route_indices) and r['max_same_image_difference']==0 and len(r['rows'])==len(route_indices)
    for offset,row in enumerate(r['rows']):
        route=route_indices[offset];assert row['route']==route;e=plan['models'][plan['routes'][route]['model_index']];assert row['entry']==e
    ...
    result=dict(status='PASS_ALL_ENDPOINT_DECODE_AND_COST_RECONSTRUCTIONS',index=r['index'],routes=len(route_indices),cases=128*len(route_indices),report_sha256=sha(d/'report.json'),source_sha256=sha(__file__),scope='All saved tokens decoded and physical/predicted costs/success reconstructed independently with NumPy. Same-image neural checks and frozen tensors are executed by the bound extraction; this verifier does not rerun neural encoding or physics.')
