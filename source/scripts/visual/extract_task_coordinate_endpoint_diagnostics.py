...
def main():
    ...
    assert plan['task_coordinate_evaluation']['status']=='READY_COMPLETE_FRESH_EVALUATION' and len(bm['cases'])==128 and sha(bank/'manifest.json')==plan['bank_manifest_sha256']
    route_indices=[i for i,r in enumerate(plan['routes']) if plan['models'][r['model_index']]['original_model_index']==a.index]
    assert len(route_indices)==(10 if a.index%2==0 else 6)
    original_entry=next(e for e in plan['models'] if e['original_model_index']==a.index and e['adaptation_condition']=='original');td=Path(original_entry['training_path']);tr=json.loads((td/'summary.json').read_text())
    ...
    report=dict(status='EXTRACTED_REQUIRES_NUMERIC_ACCEPTANCE',index=a.index,rows=rows,plan_sha256=sha(a.plan),source_sha256=sha(__file__),same_image_checks=256*len(route_indices),max_same_image_difference=0.,elapsed_seconds=time.monotonic()-start,gpu=torch.cuda.get_device_name(),scope='All prespecified routes128goals for one starting model. Original and adapted image encoders compared exactly on each same terminal/goal image; frozen tensors independently checked. Goal encoding also matches stored planner tokens within2e-5. Privileged real-terminal diagnostics, not a deployable policy; changes in visited-state errors remain possible despite identical perception.')
