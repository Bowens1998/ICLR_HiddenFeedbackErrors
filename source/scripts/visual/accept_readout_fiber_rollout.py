...
def main():
    ...
    assert r['status']=='FIBER_ROLLOUT_REQUIRES_NUMPY_ACCEPTANCE' and r['plan_sha256']==sha(a.plan) and r['protocol_sha256']==sha(a.protocol) and r['source_sha256']==sha(Path(__file__).with_name('run_readout_fiber_rollout.py'))
    ...
    result=dict(status='PASS_ALL3_SINGLE_FIBER_ROLLOUT_RECONSTRUCTIONS',index=g,rows=rows,seeds=common[1].tolist(),horizons=[5,10,15,20,25],max_initial_normalized_readout_error=maxerr,report_sha256=sha(d/'report.json'),source_sha256=sha(__file__),protocol_sha256=sha(a.protocol),scope='Independent NumPy decode/metrics and exact saved-array baseline, correction and reset identities. Full neural free trajectory anchored by extraction; modified neural trajectories are not independently reimplemented.')
