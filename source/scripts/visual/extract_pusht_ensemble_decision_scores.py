...
def main():
    ...
    with torch.inference_mode():
        for case,b in enumerate(bm['cases'][:count]):
            ...
            assert seed==b['seed'];actions=[];original=[];archives=[]
            for i,r,am in zip([rid,rid+1],accepted,manifests):
                ...
                with np.load(path,allow_pickle=False) as z:actions.append(z['selected_actions'])
            ...
            at=torch.as_tensor(actions,device='cuda',dtype=torch.float32);scores=[];tokens=[];goals=[];max_batch=0.;max_numpy=0.
            for model,e,pair,norm in zip(models,entries,heads,norms):
                ...
                cost=NonlinearPoseCost(base,*pair) if pair else base;cost.verify_native(at);value,token=cost(at)
                for k in range(2):
                    single,st=cost(at[k:k+1]);torch.testing.assert_close(st,token[k:k+1],rtol=2e-5,atol=2e-5);torch.testing.assert_close(single,value[k:k+1],rtol=2e-5,atol=2e-5)
                    max_batch=max(max_batch,float(abs(single-value[k:k+1]).max()))
            ...
            scores=np.stack(scores);np.testing.assert_allclose(scores[a.actor//2],original,rtol=2e-5,atol=2e-5)
            key=f'case_{case}';arrays[key+'_scores']=scores;arrays[key+'_tokens']=np.stack(tokens);arrays[key+'_goals']=np.stack(goals);arrays[key+'_actions']=actions
            rows.append(dict(case=case,seed=seed,input_archive_sha256=b['sha256'],selected_archive_sha256=archives,original_scores=original,max_batch_difference=max_batch,max_cpu_score_difference=max_numpy))
    ...
    root=Path(__file__).resolve().parents[2];sources=['scripts/visual/extract_pusht_ensemble_decision_scores.py','scripts/visual/factorial_model.py','scripts/visual/lewm_adapter.py','scripts/visual/image_planner_cost.py','scripts/visual/nonlinear_pose_cost.py','scripts/visual/evaluation_precision.py','scripts/visual/analyze_coverage_goals.py']
    report=dict(status='ENGINEERING_COMPLETE_REQUIRES_ACCEPTANCE' if a.engineering else 'EXTRACTED_REQUIRES_ACCEPTANCE',actor=a.actor,interface=a.interface,cases=rows,members=entries,bindings=bindings,plan_sha256=sha(a.plan),authoritative_summary_sha256=sha(a.summary),scores_sha256=sha(out/'scores.npz'),source_sha256={s:sha(root/s) for s in sources},protocol_sha256=sha(root/'docs/maintrack/ENSEMBLE_DECISION_RISK_PROTOCOL.md'),precision=precision,gpu=torch.cuda.get_device_name(),elapsed_seconds=time.monotonic()-start,peak_allocated_bytes=torch.cuda.max_memory_allocated(),scope='Consumed fixed PushT pairs; only observed history, goal image and actions used. Native and batch checks; independent acceptance pending. No new planning outcomes or risk fit.')
