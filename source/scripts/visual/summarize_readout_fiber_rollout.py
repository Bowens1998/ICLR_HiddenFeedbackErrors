...
def main():
    ...
    for g in range(6):
        ...
        assert ac['status']=='PASS_ALL3_SINGLE_FIBER_ROLLOUT_RECONSTRUCTIONS' and ac['source_sha256']==sha(Path(__file__).with_name('accept_readout_fiber_rollout.py')) and ac['report_sha256']==sha(d/'report.json')
    ...
    draws=np.random.default_rng(1361001).integers(0,128,(10000,128));groups=[];effects=[]
    ...
    result=dict(status='COMPLETE18_MODELS_SINGLE_FEEDBACK_INTERVENTION',groups=groups,contrasts=effects,bindings=bindings,seeds=seeds,horizons=[5,10,15,20,25],complete_free_trajectory_anchors=9216,readout_preservation_checks=9216,plan_sha256=sha(a.plan),protocol_sha256=sha(a.protocol),source_sha256=sha(__file__),statistics_source_sha256=sha(Path(__file__).with_name('summarize_task_coordinate_horizon.py')),bootstrap=dict(seed=1361001,replicates=10000,scope='Retrospective secondary95% marginal conditional shared-goal intervals;128 goals, not model/stream/horizon counts.',designated_mechanism_endpoint='25-action block-position MSE; earlier horizons describe propagation'),scope='Single observed-guided feedback change after first five actions. Fiber preserves immediate task readout; full reset also changes it. Neither intervention is a deployable policy.')
