"""Independent NumPy optimizer/score reconstruction and model-free simulator replay."""
import argparse,hashlib,json,sys
from pathlib import Path
import numpy as np
p=argparse.ArgumentParser()
for k in ['run','bank','selection']:p.add_argument('--'+k,required=True)
p.add_argument('--simulator');p.add_argument('--expected-cases',type=int,default=128);a=p.parse_args();run=Path(a.run);bank=Path(a.bank);r=json.loads((run/'summary.json').read_text());manifest=json.loads((bank/'manifest.json').read_text());selection=json.loads(Path(a.selection).read_text())
assert (run/'COMPLETE').exists() and len(r['cases'])==a.expected_cases
assert r['hashes']['weights']==selection['all_validation_candidates'][r['model_index']]['weights_sha256'];assert r['hashes']['bank_manifest']==hashlib.sha256((bank/'manifest.json').read_bytes()).hexdigest()
...
mn=np.array(r['target_normalization']['mean']);sd=np.array(r['target_normalization']['std']);max_replay=0.;max_score=0.
for row,item in zip(r['cases'],manifest['cases']):
 ...
 dim=2 if r['parameterization']=='held' else 10;params=z['parameters'];costs=z['costs'];assert params.shape==(30,300,5,dim) and costs.shape==(30,300) and z['tokens'].shape==(30,300,6);assert np.isfinite(costs).all() and np.max(abs(params))<=np.float32(.35)
 state=z['tokens'].astype(float)*sd+mn;goal=truth['goal_state'];angle=np.arctan2(state[...,4],state[...,5])-goal[4];angle=(angle+np.pi)%(2*np.pi)-np.pi;rebuilt=np.sum((state[...,2:4]-goal[2:4])**2,-1)+900*angle**2;np.testing.assert_allclose(rebuilt,costs,rtol=5e-5,atol=2e-3);max_score=max(max_score,float(np.max(abs(rebuilt-costs))))
