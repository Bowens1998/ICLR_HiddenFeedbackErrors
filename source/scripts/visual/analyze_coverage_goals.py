"""Complete-matrix paired goal manipulation check; no planner outcome inference."""
import argparse
import hashlib
import json
...
def summarize(values, indices):
    ...
    assert values.shape == (256,) and np.isfinite(values).all()
...
def main():
    ...
    for name in ['evaluations', 'fits', 'bank', 'plan', 'config', 'output']:
    ...
    assert ba['status'] == 'PASS' and ba['cases'] == len(bm['cases']) == 256
    assert ba['manifest_sha256'] == sha(bank/'manifest.json')
    assert ba['config_sha256'] == sha(a.config)
    assert ba['verifier_sha256'] == sha(Path(__file__).with_name('accept_coverage_goals.py'))
    assert bm['seed_start'] == cfg['evaluation_seed_start']
    truth, seeds = [], []
    for i, item in enumerate(bm['cases']):
        f = bank/f'case_{i:03d}.npz'
        assert item['index'] == i and sha(f) == item['sha256']
        with np.load(f) as z:
            state = z['goal_state']
            truth.append(np.r_[state[:4], np.sin(state[4]), np.cos(state[4])])
    ...
    indices = np.random.default_rng(1253901).integers(0, 256, size=(10000, 256))
    ...
    for i in range(6):
        ...
        assert r['status'] == 'COMPLETE_FROZEN_GOAL_EVALUATION' and r['index'] == fr['index'] == i
        ...
        assert r['source_sha256'] == sha(Path(__file__).with_name('evaluate_coverage_goals.py'))
        assert r['plan_sha256'] == fr['plan_sha256'] == sha(a.plan)
        assert r['config_sha256'] == fr['config_sha256'] == sha(a.config)
        assert r['bank_manifest_sha256'] == sha(bank/'manifest.json')
        ...
        with np.load(run/'goals.npz') as z:
            np.testing.assert_array_equal(z['seed'], seeds)
            np.testing.assert_array_equal(z['target'], truth)
            tokens = z['tokens']
        assert tokens.shape == (256, 192) and np.isfinite(tokens).all()
        ...
        for row in r['rows']:
            ...
            with np.load(run/row['file']) as z:
                pred = z['prediction']
            assert pred.shape == (256, 6) and np.isfinite(pred).all()
    ...
    result = dict(status='PASS_COMPLETE_MATRIX', models=6, heads=18, goals=256,
                  primary_gate_pass=contrasts['joint_precision']['ci95'][0] > 0,
                  primary='equal-six-backbone broad-minus-expert joint precision; lower paired95CI > 0',
                  bootstrap=dict(draws=10000, seed=1253901, unit='shared goal', interval='percentile'),
                  ...
                  bank_manifest_sha256=sha(bank/'manifest.json'), config_sha256=sha(a.config),
                  source_sha256=sha(__file__), scope='Conditional on six fixed backbones and three sampled training pools. Secondary intervals nominal, no multiplicity correction. Goal localization only; no planning claim. Original reference has a larger label budget. Saved predictions independently recomputed; goal encodings inherit native evaluation and are not independently re-encoded here.')
    out = Path(a.output)
    out.parent.mkdir(parents=True, exist_ok=True)
    with out.open('x') as f:
        f.write(json.dumps(result, indent=2)+'\n')
    print('PASS_COMPLETE_MATRIX', 'PRIMARY_GATE', result['primary_gate_pass'])
