...
def main():
    ...
    for k in ('run','cache','freeze','official','config','protocol'):p.add_argument('--'+k,required=True)
    a=p.parse_args();d=Path(a.run);r=json.loads((d/'report.json').read_text());e=r['entry'];cache=Path(a.cache)/f"job_{r['model_index']}"
    assert r['status']=='TRAINED_REQUIRES_INDEPENDENT_ACCEPTANCE' and r['source_sha256']==sha(Path(__file__).with_name('train_task_coordinate_formal.py'))
    ...
    objective=('latent','decoded_teacher','physical_labels','native_state')[r['index']%4]
    assert r['objective']==objective and r['model_index']==2*(r['index']//4)+(objective=='native_state') and r['stream']=='planner'
    assert r['gradient_control']=='unit_global_norm_before_adamw' and r['protocol_sha256']==sha(a.protocol)
    assert r['control_source_sha256']==sha(Path(__file__).with_name('adaptation_gradient_control.py')) and r['loss_source_sha256']==sha(Path(__file__).with_name('adaptation_task_losses.py'))
    assert len(r['unit_gradient_norms'])==len(r['optimizer_update_norms'])==2100
    assert max(abs(x-1) for x in r['unit_gradient_norms'])<1e-6 and np.isfinite(r['optimizer_update_norms']).all() and min(r['optimizer_update_norms'])>0
    head=None
    if e['score']!='state':
    ...
    assert r['updates']==2100 and r['batch_size']==128 and r['learning_rate']==1e-5 and r['weight_decay']==1e-3 and r['clip_norm'] is None
    ...
    for phase,state in [('before',original),('after',updated)]:
        ...
        for row in cr['rows']:
            ...
            objective_prediction=reference.astype(float);objective_target=z['target'].astype(float)
            if objective in ('decoded_teacher','physical_labels'):
                objective_prediction=(decode(objective_prediction,head)-head['target_mean'])/head['target_scale']
                if objective=='decoded_teacher':objective_target=(decode(z['observed'][:,1:],head)-head['target_mean'])/head['target_scale']
                else:
                    states=z['raw_states'].astype(float);physical=np.concatenate([states[...,:4],np.sin(states[...,4:5]),np.cos(states[...,4:5])],axis=-1);objective_target=((physical-head['target_mean'])/head['target_scale'])[:,1:]
            objective_loss=weight*np.mean((objective_prediction-objective_target)**2)
            np.testing.assert_allclose(objective_loss,r['objective_metrics'][row['stream']][phase],rtol=2e-5,atol=2e-5)
    ...
    result=dict(status='PASS_TASK_COORDINATE_FORMAL_FIT_AND_CPU_PREDICTIONS',index=r['index'],rows=rows,report_sha256=sha(d/'report.json'),source_sha256=sha(__file__),scope='Every before/after cached-input temporal prediction reconstructed on CPU and objective targets/losses independently rebuilt with NumPy; original/final checkpoints reloaded, all frozen tensors/buffers and unchanged head hashes checked. Optimizer trajectory not independently rerun. Full source-image cache reconstruction is separately accepted; no actual planning utility.')
