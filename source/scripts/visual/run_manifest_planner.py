...
frozen=json.loads(Path(a.model_manifest).read_text());route=frozen['routes'][a.index];model_index=route['model_index'];entry=frozen['models'][model_index];arm=entry['arm'];assert arm in ARMS;algorithm=route['algorithm'];parameterization=route['parameterization']
