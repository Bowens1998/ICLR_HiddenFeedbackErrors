...
(out/'report.json').write_text(json.dumps(dict(rows=report,gpu=torch.cuda.get_device_name(),scope='single fixed minibatch, same GPU, deterministic algorithms and math SDPA, initialized CUDA; does not establish213-step equivalence'),indent=2)+'\n')
