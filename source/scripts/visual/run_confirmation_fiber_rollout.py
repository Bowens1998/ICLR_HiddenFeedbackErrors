...
def main():
    ...
    with torch.inference_mode():
        for slot in (2,3,4):
            ...
            assert fac['status']=='PASS1536_PAIRED_CONFIRMATION_FIBER_INPUTS' and fac['constrained_valid']==fac['shuffled_valid']==1536 and fac['report_sha256']==sha(fd/'report.json') and fac['source_sha256']==sha(Path(__file__).with_name('accept_confirmation_fiber_inputs.py'))
            assert fr['protocol_sha256']==protocol_sha and fr['source_sha256']==sha(Path(__file__).with_name('prepare_confirmation_fiber_inputs.py'))
            ...
            values={path:[[] for _ in refs] for path in ('free','fiber','shuffled','full','reset')};goals=[]
            for i,h in enumerate(histories):
                ...
                for k,examples in enumerate(refs):
                    ...
                    for key in ('constrained','shuffled','full'):
                        err=abs((physical(fz[key][j])-physical(fz['predicted'][j]))/head['target_scale']);assert np.max(err)<=1e-6;readout_checks+=1
                    replacements={'full':torch.as_tensor(fz['full'][j],device='cuda'),'shuffled':torch.as_tensor(fz['shuffled'][j],device='cuda'),'fiber':torch.as_tensor(correction,device='cuda'),'reset':torch.as_tensor(fz['observed'][j],device='cuda')}
                    for path in values:
                        ...
                        else:np.testing.assert_array_equal(tokens[0],fz[{'fiber':'constrained','shuffled':'shuffled','full':'full','reset':'observed'}[path]][j])
    ...
    assert anchor_checks==1536 and readout_checks==4608 and goal_checks==384 and protocol_sha==sha(a.protocol)
    sources={p:sha(Path(__file__).with_name(p)) for p in ('run_confirmation_fiber_rollout.py','image_planner_cost.py','factorial_model.py','evaluation_precision.py','adaptation_freeze.py','extract_pose_selected_endpoints.py','lewm_adapter.py','accept_task_coordinate_horizon.py','accept_confirmation_fiber_inputs.py','prepare_confirmation_fiber_inputs.py','readout_fiber.py')}
    report=dict(status='CONFIRMATION_FIBER_ROLLOUT_REQUIRES_NUMPY_ACCEPTANCE',index=a.index,rows=rows,reference_bindings=reference_bindings,anchor_checks=anchor_checks,readout_checks=readout_checks,goal_checks=goal_checks,input_bindings=input_bindings,plan_sha256=sha(a.plan),protocol_sha256=protocol_sha,source_sha256=sha(__file__),sources=sources,elapsed_seconds=time.monotonic()-started,gpu=torch.cuda.get_device_name(),scope='Independent-goal confirmation with matched guidance, external-donor guidance and full correction at the first predicted block: free, matched and shuffled readout-preserving corrections, and full observed reset. All3 JEPA objectives,128goals and4fixed references. Exact complete free-trajectory anchors; no new training/search or later observed feedback.')
