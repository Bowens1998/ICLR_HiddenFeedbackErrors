"""Native image-only candidate scoring for both JEPA and privileged-supervised references."""
...
def main():
    ...
    precision=configure_evaluation_precision();model=make_model(a.official,a.config,r['arm'],r['seed']);weights=run/f'{a.checkpoint}_weights.pt'
    ...
    with torch.inference_mode():
        for item in manifest['cases']:
            ...
            act=(torch.from_numpy(act).cuda()-am)/sd;history=pixels(z['history_pixels'])[None,None].expand(1,n,-1,-1,-1,-1)
            ...
            goal=encode(z['goal_pixels'][None])[0].cpu().numpy();predicted=native[:,-1].cpu().numpy();realized=encode(z['terminal_pixels']).cpu().numpy()
            scores={'native':token_scores(predicted,goal,r['arm'],r['target_normalization']),
                    'oracle_realized':token_scores(realized,goal,r['arm'],r['target_normalization'])}
    ...
    summary={'arm':r['arm'],'checkpoint':a.checkpoint,'training_seed':r['seed'],'gpu':torch.cuda.get_device_name(),'precision':precision,'cases':rows,'target_normalization':r['target_normalization'],
             'scope':'same-data development; state arms receive extra training labels; no ground-truth evaluation states in deployed scores',
