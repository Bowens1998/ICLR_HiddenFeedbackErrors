...
def main():
    ...
    for row in r['cases']:
        ...
        np.testing.assert_allclose(scores[r['actor']//2],row['original_scores'],rtol=2e-5,atol=2e-5)
    output=dict(status='PASS',cases=count,actor=r['actor'],interface=r['interface'],max_cpu_score_difference=max_error,report_sha256=sha(folder/'report.json'),scores_sha256=sha(folder/'scores.npz'),verifier_sha256=sha(__file__),scope='Independent NumPy head/angle arithmetic and saved identities; neural encoder/predictor and original archive loading checks remain extractor checks. No independent neural replay, optimizer replay, risk fit or new outcome evidence.')
