...
def main():
        ...
        for slot in range(8):
            ...
            for i,h in enumerate(histories):
                ...
                for k,examples in enumerate(refs):
                    x=examples[i];actions=torch.as_tensor(x['population_actions'],device='cuda');_,tokens=cost(actions);token=tokens[x['selected_index']].cpu().numpy();assert np.isfinite(token).all()
                    if reference_bindings[k]['model_index']==mi:
                        print('ANCHOR',mi,i,reference_routes[k],float(np.max(abs(token-x['actor_tokens']))),flush=True)
                        np.testing.assert_array_equal(token,x['actor_tokens']);anchor_checks+=1
    ...
    sources={p:sha(Path(__file__).with_name(p)) for p in ('run_task_coordinate_matched_populations.py','image_planner_cost.py','factorial_model.py','evaluation_precision.py','adaptation_freeze.py','extract_pose_selected_endpoints.py','lewm_adapter.py')}
    report=dict(status='MATCHED_ACTION_EXTRACTION_REQUIRES_NUMPY_ACCEPTANCE',index=a.index,rows=rows,reference_bindings=reference_bindings,anchor_checks=anchor_checks,same_image_checks=same_image_checks,plan_sha256=sha(a.plan),protocol_sha256=protocol_sha,source_sha256=sha(__file__),sources=sources,elapsed_seconds=time.monotonic()-started,gpu=torch.cuda.get_device_name(),scope='All8 conditions on four fixed original-policy action streams128goals. Original-population300 diagnostic with fixed selected index, no search or new physical labels.512 original-actor token anchors and3840 continued/original same-image comparisons exact. Frozen tensors checked; independent NumPy acceptance still required.')
