...
def main():
    ...
    assert r['status']=='UPDATED_REQUIRES_CPU_ACCEPTANCE' and r['source_sha256']==sha(Path(__file__).with_name('run_task_coordinate_gate.py')) and r['loss_source_sha256']==sha(Path(__file__).with_name('adaptation_task_losses.py'))
    ...
    report=dict(status='PASS_ALL_THREE_TASK_COORDINATE_ENGINEERING_OBJECTIVES',index=r['index'],rows=rows,report_sha256=sha(d/'report.json'),source_sha256=sha(__file__),scope='CPU original/final prediction reconstruction, bytewise frozen state and exact allowed changes; independent NumPy targets/initial losses. Does not rerun gradient/update trajectory or establish utility.')
