...
def main():
    ...
    with torch.inference_mode():
        for slot in (2,3,4):
            ...
            assert fac['status']=='PASS1536_MATCHED_FIBER_INPUTS' and fac['constrained_valid']==1536 and fac['report_sha256']==sha(fd/'report.json') and fac['source_sha256']==sha(Path(__file__).with_name('accept_matched_fiber_inputs.py'))
            assert fr['protocol_sha256']==protocol_sha and fr['source_sha256']==sha(Path(__file__).with_name('prepare_matched_fiber_inputs.py'))
    ...
    sources={p:sha(Path(__file__).with_name(p)) for p in ('run_matched_fiber_rollout.py','image_planner_cost.py','factorial_model.py','evaluation_precision.py','adaptation_freeze.py','extract_pose_selected_endpoints.py','lewm_adapter.py','accept_task_coordinate_horizon.py','accept_matched_fiber_inputs.py','prepare_matched_fiber_inputs.py','readout_fiber.py')}
    report=dict(status='MATCHED_FIBER_ROLLOUT_REQUIRES_NUMPY_ACCEPTANCE',index=a.index,rows=rows,reference_bindings=reference_bindings,anchor_checks=anchor_checks,readout_checks=readout_checks,goal_checks=goal_checks,input_bindings=input_bindings,plan_sha256=sha(a.plan),protocol_sha256=protocol_sha,source_sha256=sha(__file__),sources=sources,elapsed_seconds=time.monotonic()-started,gpu=torch.cuda.get_device_name(),scope='Matched-displacement single feedback intervention at the first predicted block: free, readout-preserving fiber correction and full observed reset. All3 JEPA objectives,128goals and4fixed references. Exact complete free-trajectory anchors; no new training/search or later observed feedback.')
