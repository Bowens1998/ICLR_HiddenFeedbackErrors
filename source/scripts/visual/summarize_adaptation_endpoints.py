...
def main():
    ...
    assert evaluation['status']=='COMPLETE72_FRESH_ADAPTATION_ROUTES' and evaluation['plan_sha256']==sha(a.plan)
    ...
    for index in range(12):
        ...
        assert ac['source_sha256']==sha(Path(__file__).with_name('accept_adaptation_endpoint_diagnostics.py'))
        assert r['index']==index and r['plan_sha256']==sha(a.plan) and r['same_image_checks']==1536 and r['max_same_image_difference']==0 and len(r['rows'])==6
        ...
        for j,item in enumerate(r['rows']):
            route=index*6+j;assert item['route']==route;e=plan['models'][plan['routes'][route]['model_index']];assert item['entry']==e
