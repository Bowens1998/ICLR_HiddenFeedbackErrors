"""Prespecified shared-goal secondary confirmation horizon contrasts for all frozen models."""
...
def main():
    ...
    draws=np.random.default_rng(1368001).integers(0,128,(20000,128));groups=[];contrasts=[]
    ...
    result=dict(status='COMPLETE48_MODELS_FRESH_SECONDARY_HORIZONS',groups=groups,contrasts=contrasts,bindings=bindings,seeds=seeds,horizons=[5,10,15,20,25],endpoint_anchor_checks=24576,first_step_checks=24576,observed_terminal_checks=24576,same_image_checks=96768,plan_sha256=sha(a.plan),protocol_sha256=sha(a.protocol),source_sha256=sha(__file__),bootstrap=dict(seed=1368001,replicates=20000,unit='128 shared goals; models, streams and horizons not independent replicates',scope='Prespecified secondary conditional95% marginal percentile intervals on fresh recipient goals; not part of the five primary contrasts.'),scope='Free versus privileged actual-history feedback on fixed original-policy action streams; no new training or search. All six model points retained.')
