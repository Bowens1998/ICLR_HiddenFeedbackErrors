...
def main():
    ...
    for k in ('cache','freeze','official','config','output'):p.add_argument('--'+k,required=True)
    p.add_argument('--index',type=int,choices=range(24),required=True);a=p.parse_args()
    index=a.index//2;stream=('expert','planner')[a.index%2];cache=Path(a.cache)/f'job_{index}'
    ...
    for row in r['rows']:
        ...
        datasets[row['stream']]={k:torch.as_tensor(z[k],device='cuda') for k in ('observed','normalized_actions','target')}
    ...
    z=datasets[stream+'_train'];assert len(z['observed'])==1280;losses=[];gradients=[]
    for step,indices in enumerate(schedule):
        ...
        pred=temporal(model,e['arm'],z['observed'][ix],z['normalized_actions'][ix]);loss=weight*(pred-z['target'][ix]).square().mean();assert torch.isfinite(loss)
        loss.backward();gradient=torch.nn.utils.clip_grad_norm_(parameters,1.);assert torch.isfinite(gradient);optimizer.step();verify_frozen(model,boundary)
        losses.append(float(loss.detach()));gradients.append(float(gradient))
