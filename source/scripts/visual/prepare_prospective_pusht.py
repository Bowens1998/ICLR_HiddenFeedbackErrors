"""Prospective nontrivial block-transport goals; no model scores used for selection."""
import argparse,hashlib,json,sys
from pathlib import Path
import numpy as np

def main():
    p=argparse.ArgumentParser();p.add_argument('--source',required=True);p.add_argument('--output',required=True);p.add_argument('--cases',type=int,default=32);p.add_argument('--seed-start',type=int,required=True);p.add_argument('--max-seeds',type=int,required=True);a=p.parse_args();sys.path.insert(0,a.source)
    ...
    def visible(e):
        ...
        return True
    for seed in range(a.seed_start,a.seed_start+a.max_seeds):
        ...
        goal=None;goal_rejections={'visibility':0,'displacement':0}
        for goal_attempt in range(1,129):
            goal_plan=sample(rng_goal);goal=replay(goal_plan)
            if goal is None:goal_rejections['visibility']+=1;continue
            if np.linalg.norm(goal[3][2:4]-goal[1][-1,2:4])<40:
                goal_rejections['displacement']+=1;goal=None;continue
            break
        if goal is None:rejected.append({'seed':seed,'reason':'no_visible_displaced_goal_in_128_attempts','goal_rejections':goal_rejections});continue
        ...
        row={'index':index,'seed':seed,'sha256':hashlib.sha256(path.read_bytes()).hexdigest(),'goal_attempts':goal_attempt,'goal_rejections':goal_rejections,'initial_goal_block_distance':float(np.linalg.norm(goal[3][2:4]-goal[1][-1,2:4])),'candidate_attempts':attempts,
    ...
    manifest={'scope':'prospective visible block-transport goals; separate validation/held-out streams; not the official task distribution','cases':rows,'candidates':32,'rejected_seeds':rejected,
              'primitive_action_limit':.35,'visibility_box':[8,504],'seed_start':a.seed_start,'max_seeds':a.max_seeds,'minimum_goal_block_displacement':40,'max_attempts_per_slot':128,
