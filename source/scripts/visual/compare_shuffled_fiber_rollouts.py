"""Exact unchanged branches and full-versus-shuffled paired intervention effects."""
...
def main():
    ...
    for k in ('full','shuffled','output'):p.add_argument('--'+k,required=True)
    a=p.parse_args();data={};bindings=[];checks=0;seeds=None
    for g in range(6):
        pair={}
        for tag,base in [('full',a.full),('shuffled',a.shuffled)]:
            ...
            assert ac['source_sha256']==sha(Path(__file__).with_name('accept_'+('readout' if tag=='full' else 'shuffled')+'_fiber_rollout.py'))
        ...
        for c in range(3):
            ...
            for key in ('free_tokens','free_pose','reset_tokens','reset_pose','observed_tokens','observed_pose','true_pose','goal_tokens','seeds','reference_routes','horizons'):np.testing.assert_array_equal(zz['full'][key],zz['shuffled'][key]);checks+=1
    ...
    for c,name in enumerate(['unit_latent','unit_decoded_teacher','unit_physical_labels']):
        ...
        for metric in data[0,c,'full']:
            full=np.stack([data[g,c,'full'][metric].mean(0) for g in range(6)]);shuffled=np.stack([data[g,c,'shuffled'][metric].mean(0) for g in range(6)]);mm[metric]=contrast(shuffled,full,draws)
        rows.append(dict(condition=name,left='shuffled',right='full',metrics=mm))
