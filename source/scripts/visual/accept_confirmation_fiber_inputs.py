...
def main():
    ...
    for k in ('run','horizon','full','protocol','donor-horizon'):p.add_argument('--'+k,required=True)
    ...
    assert r['status']=='CONFIRMATION1536_PAIRED_FIBER_INPUTS_REQUIRES_ACCEPTANCE' and r['source_sha256']==sha(Path(__file__).with_name('prepare_confirmation_fiber_inputs.py')) and r['protocol_sha256']==sha(a.protocol) and r['projection_source_sha256']==sha(Path(__file__).with_name('readout_fiber.py'))
    ...
    dd=Path(a.donor_horizon)/f'job_{g}';dr=json.loads((dd/'report.json').read_text());dac=json.loads((dd/'acceptance.json').read_text())
    assert sha(dd/'report.json')==r['donor_report_sha256'] and sha(dd/'acceptance.json')==r['donor_acceptance_sha256'] and dac['report_sha256']==r['donor_report_sha256'] and dac['status']=='PASS_ALL8_HORIZON_NUMPY_RECONSTRUCTIONS'
    assert dr['plan_sha256']=='1fd34ed8b462afc5901b6d82c9ef119ff689424be03803b92882064a5559c79b'
    rng=np.random.default_rng(1368001);permutation=rng.permutation(128)
    ...
    for slot in (2,3,4):
        ...
        np.testing.assert_array_equal(s['predicted'],z['free_tokens'][:,:,0].reshape(512,192));np.testing.assert_array_equal(s['observed'],z['observed_tokens'][:,:,0].reshape(512,192));np.testing.assert_array_equal(s['full'],f['constrained']);np.testing.assert_array_equal(s['permutation'],permutation);drow=dr['rows'][slot];db=r['donor_rows'][slot-2];assert db['model_index']==mi and db['file']==drow['file'] and db['sha256']==drow['sha256']==sha(dd/drow['file']) and drow['head_sha256']==b['head_sha256'];dz=np.load(dd/drow['file']);assert not set(dz['seeds'])&set(z['seeds']);np.testing.assert_array_equal(s['donor'],dz['observed_tokens'][:,:,0][:,permutation].reshape(512,192))
        nn={}
    ...
    result=dict(status='PASS1536_PAIRED_CONFIRMATION_FIBER_INPUTS',index=g,constrained_valid=1536,shuffled_valid=1536,rows=rows,max_constrained_readout_error=max(x['max_scaled_readout_error'] for x in rows),max_region_violation=max(x['max_region_violation'] for x in rows),report_sha256=sha(d/'report.json'),source_sha256=sha(__file__),protocol_sha256=sha(a.protocol))
