...
def main():
    ...
    for k in ['run','bank','plan','simulator','output']:p.add_argument('--'+k,required=True)
    ...
    phase='adaptation_formal' if 'adaptation_formal' in plan else 'adaptation_engineering'
    assert am['summary.json']==sha(d/'summary.json') and r['route_index'] in plan[phase]['execute_routes']
    if phase=='adaptation_formal':
        split=plan[phase]['split'];assert split in ['train','validation']
        assert len(bm['cases'])=={'train':128,'validation':32}[split]
    ...
    if phase=='adaptation_formal':
        report['split']=split
        report['scope']='Formal adaptation '+split+' trajectories: eight stride-five observed frames,35 actions,36 states. All admitted selected plans retained regardless of cost/success/boundary. No fitted model or evaluation result.'
