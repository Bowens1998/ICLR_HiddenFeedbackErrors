...
p.add_argument('--simulator');p.add_argument('--expected-cases',type=int,default=128);a=p.parse_args();run=Path(a.run);bank=Path(a.bank);r=json.loads((run/'summary.json').read_text());manifest=json.loads((bank/'manifest.json').read_text());selection=json.loads(Path(a.model_manifest).read_text());assert selection['layout']=='pusht_nonlinear_pose'
assert r['verification_policy']=='exact_original_population_replay_with_single_action_diagnostic'
assert r['verification_protocol_sha256']==hashlib.sha256((run/'source/TASK_COORDINATE_POPULATION_REPLAY_PROTOCOL.md').read_bytes()).hexdigest()
assert (run/'COMPLETE').exists() and len(r['cases'])==a.expected_cases
route=selection['routes'][r['route_index']];entry=selection['models'][route['model_index']]
assert r['model_index']==route['model_index'] and r['algorithm']==route['algorithm'] and r['parameterization']==route['parameterization']
assert r['score_space']==entry['score'] and r['mode']==entry['mode']
assert r['checkpoint']==entry['checkpoint']=='last' and r['training_path']==entry['training_path']
...
heads={}
if r['score_space'].startswith('pose_'):
...
def decode(tokens,h):
 x=(tokens-h['mean'])/h['scale']
 for layer in [0,2,4]:
  x=x@h[f'{layer}.weight'].astype(float).T+h[f'{layer}.bias'].astype(float)
...
assert r['target_normalization']==entry['target_normalization']
...
for row,item in zip(r['cases'],manifest['cases']):
 ...
 elif r['score_space'].startswith('pose_'):
  endpoint=decode(tokens,heads['endpoint_head']);gp=decode(goal_token,heads['goal_head']);angle=np.arctan2(endpoint[...,4],endpoint[...,5])-np.arctan2(gp[4],gp[5]);angle=np.angle(np.exp(1j*angle));rebuilt=np.square(endpoint[...,2:4]-gp[2:4]).sum(-1)+900*angle**2;atol=1e-8
 else:
 ...
 np.testing.assert_allclose(rebuilt,costs,rtol=1e-10 if r['score_space'].startswith('pose_') else 5e-5,atol=atol);max_score=max(max_score,float(np.max(abs(rebuilt-costs))))
 ...
 ti_saved=row['selected_iteration']
 np.testing.assert_array_equal(z['population_replay_tokens'],z['tokens'][ti_saved]);np.testing.assert_array_equal(z['population_replay_costs'],costs[ti_saved]);assert row['population_replay_exact']
 selected=z['tokens'][ti_saved,row['selected_candidate']];single=z['single_tokens'];assert np.isfinite(single).all() and np.isfinite(z['single_cost']).all()
 delta=abs(single-selected);assert bool((delta<=2e-5+2e-5*abs(selected)).all())==row['single_matches_original_tolerance']
 assert float(delta.max())==row['single_max_token_difference'] and float(z['single_cost'])-row['predicted_cost']==row['single_cost_difference']
 ...
 actions=np.repeat(chosen,5,axis=0) if dim==2 else chosen.reshape(25,2);np.testing.assert_array_equal(actions,z['selected_actions']);assert row['scored_candidates']==9000 and row['additional_verification_forwards']==2 and row['additional_verification_candidates']==301
 def physical(s):
...
result={'verification_protocol_sha256':r['verification_protocol_sha256'],'population_replay_exact_all_cases':True,'single_original_tolerance_failures':sum(not row['single_matches_original_tolerance'] for row in r['cases']),'cases':len(r['cases']),'candidate_scores_per_case':9000,'scores_CEM_updates_choices_actions_metrics_hashes':'independently reconstructed','max_absolute_float32_score_reconstruction_difference':max_score,'model_free_simulator_replay':bool(a.simulator),'max_replay_state_difference':max_replay if a.simulator else None,'verifier_sha256':hashlib.sha256(Path(__file__).read_bytes()).hexdigest()}
