"""Absorb fixed normalization into first layer; training-only optimization control."""
...
def main():
    ...
    cfg=json.loads(Path(a.config).read_text());assert cfg['parameterization']=='normalization_absorbed_first_layer';plan=json.loads(Path(a.plan).read_text());entry=plan['models'][plan['routes'][a.index*8+2]['model_index']]
    ...
    head=nn.Sequential(nn.Linear(192,256),nn.ReLU(),nn.Linear(256,256),nn.ReLU(),nn.Linear(256,6)).cuda()
    head.load_state_dict({k:torch.tensor(v,device='cuda') for k,v in initial_head.items()},strict=True)
    # Exact algebra in float64, then quantify float32 storage cancellation.
    original_mean=h['mean'].copy();original_scale=h['scale'].copy()
    w=initial_head['0.weight'].astype(float)/original_scale[None,:]
    b=initial_head['0.bias'].astype(float)-w@original_mean
    original_head=nn.Sequential(nn.Linear(192,256),nn.ReLU(),nn.Linear(256,256),nn.ReLU(),nn.Linear(256,6)).cuda()
    original_head.load_state_dict({k:torch.tensor(v,device='cuda') for k,v in initial_head.items()},strict=True)
    with torch.no_grad():
        head[0].weight.copy_(torch.tensor(w,device='cuda',dtype=torch.float32));head[0].bias.copy_(torch.tensor(b,device='cuda',dtype=torch.float32))
        actual=torch.tensor(z[a.stage],device='cuda',dtype=torch.float32)
        reference=original_head(((actual.double()-torch.tensor(original_mean,device='cuda'))/torch.tensor(original_scale,device='cuda')).float())
        changed=head(actual);initial_output_gap=float((reference-changed).abs().max())
        torch.testing.assert_close(changed,reference,rtol=2e-4,atol=2e-4)
    initial_head={k:v.detach().cpu().numpy().copy() for k,v in head.state_dict().items()}
    del original_head
    ...
    h['mean']=np.zeros_like(original_mean);h['scale']=np.ones_like(original_scale)
    mean=torch.zeros_like(mean);scale=torch.ones_like(scale)
    def pixels(ix):
