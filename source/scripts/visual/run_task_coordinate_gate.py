...
def main():
    ...
    for objective in OBJECTIVES:
        ...
        for step in range(10):
            ...
            norm=torch.nn.utils.clip_grad_norm_(params,1.);assert torch.isfinite(norm);optimizer.step();verify_frozen(model,boundary)
            steps.append(dict(loss=float(loss.detach()),unclipped_gradient_norm=float(norm),clipped=bool(norm>1),roots=root_stats))
    ...
    report=dict(status='UPDATED_REQUIRES_CPU_ACCEPTANCE',index=a.index,original_model_index=index,entry=e,rows=rows,initial_gradient_cosines=cosines,engineering_report_sha256=sha(folder/'report.json'),engineering_forward_sha256=sha(fp),precision=precision,source_sha256=sha(__file__),loss_source_sha256=sha(Path(__file__).with_name('adaptation_task_losses.py')),elapsed_seconds=time.monotonic()-start,scope='Three objectives, ten20-window updates each on two engineering contexts. Finite nonzero gradients and allowed parameter changes required; head/encoder/buffers frozen. Loss units and clipping differ; no scaling coefficient selected or validation/planning result.')
