...
def main():
    ...
    for k in ('horizon','full','protocol','output','donor-horizon'):p.add_argument('--'+k,required=True)
    ...
    assert dr['plan_sha256']=='1fd34ed8b462afc5901b6d82c9ef119ff689424be03803b92882064a5559c79b' and dac['status']=='PASS_ALL8_HORIZON_NUMPY_RECONSTRUCTIONS' and dac['report_sha256']==sha(dd/'report.json')
    ...
    for slot in (2,3,4):
        ...
        h=dict(np.load(hd/row['head_file']));z=dict(np.load(fd/b['output_file']));drow=dr['rows'][slot];assert drow['head_sha256']==b['head_sha256'] and sha(dd/drow['file'])==drow['sha256'];dz=dict(np.load(dd/drow['file']));hz=np.load(hd/row['file']);assert not set(dz['seeds'])&set(hz['seeds']);donor=dz['observed_tokens'][:,:,0][:,permutation].reshape(512,192);shuffled=[];solvers=[]
    ...
    result=dict(status='CONFIRMATION1536_PAIRED_FIBER_INPUTS_REQUIRES_ACCEPTANCE',index=g,bindings=bindings,rows=rows,permutation=permutation.tolist(),full_report_sha256=sha(fd/'report.json'),full_acceptance_sha256=sha(fd/'acceptance.json'),protocol_sha256=sha(a.protocol),source_sha256=sha(__file__),projection_source_sha256=sha(Path(__file__).with_name('readout_fiber.py')),elapsed_seconds=time.monotonic()-start,donor_report_sha256=sha(dd/'report.json'),donor_acceptance_sha256=sha(dd/'acceptance.json'),donor_rows=[dict(model_index=dr['rows'][slot]['model_index'],file=dr['rows'][slot]['file'],sha256=dr['rows'][slot]['sha256']) for slot in (2,3,4)])
