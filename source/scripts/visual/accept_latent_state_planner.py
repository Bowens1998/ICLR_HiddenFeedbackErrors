...
for row,item in zip(r['cases'],manifest['cases']):
 ...
 state=decode_head(tokens,head)*sd+mn;estimated_goal=decode_head(goal_token,head)*sd+mn;angle=np.arctan2(state[...,4],state[...,5])-np.arctan2(estimated_goal[4],estimated_goal[5]);angle=(angle+np.pi)%(2*np.pi)-np.pi;rebuilt=np.sum((state[...,2:4]-estimated_goal[2:4])**2,-1)+900*angle**2;atol=2e-3
