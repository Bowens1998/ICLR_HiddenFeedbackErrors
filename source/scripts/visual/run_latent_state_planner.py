...
from latent_state_model import make_model,ARMS
from lewm_adapter import build
from latent_state_planner_cost import LatentStatePlannerCost
...
model_index=a.index//2;arm=ARMS[model_index];algorithm,parameterization=[('random','full'),('cem','full')][a.index%2]
...
run=Path(a.training)/arm;r=json.loads((run/'summary.json').read_text());assert hashlib.sha256((run/'summary.json').read_bytes()).hexdigest()==entry['training_summary_sha256'];checkpoint=entry.get('checkpoint','best');assert checkpoint in ['best','last'];weights=run/(checkpoint+'_weights.pt');model=make_model(a.official,a.config,arm,r['seed']);model.load_state_dict(torch.load(weights,map_location='cpu',weights_only=True));norm=r['normalization'];target_norm=r['target_normalization']
wh=hashlib.sha256(weights.read_bytes()).hexdigest();assert wh==entry['weights_sha256'];model=model.cuda().eval();score_space='state_head'
np.savez(out/'head_weights.npz',**{k:v.detach().cpu().numpy() for k,v in model.state_head.state_dict().items()})
source=out/'source';source.mkdir()
for path in [Path(__file__),Path(__file__).with_name('controlled_search.py'),Path(__file__).with_name('image_planner_cost.py'),Path(__file__).with_name('latent_state_planner_cost.py'),Path(__file__).with_name('latent_state_model.py'),Path(__file__).with_name('lewm_adapter.py'),Path(a.official)/'jepa.py',Path(a.official)/'module.py',Path(a.config),Path(__file__).with_name('factorial_model.py'),Path(__file__).with_name('evaluation_precision.py'),Path(a.simulator)/'stable_worldmodel/envs/pusht/env.py']:(source/path.name).write_bytes(path.read_bytes())
...
with torch.inference_mode():
 for item in manifest['cases'][:a.cases]:
  ...
  cost_function=LatentStatePlannerCost(model,z['history_pixels'],z['goal_pixels'],z['prefix'],norm,target_norm)
  if not rows:
