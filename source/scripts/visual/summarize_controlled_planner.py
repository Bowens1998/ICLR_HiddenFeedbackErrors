...
for mi,index in enumerate([1,3,4]):
 costs={}
 for pi,(algorithm,parameterization) in enumerate([('random','held'),('random','full'),('cem','held'),('cem','full')]):
  ...
  values=np.array([v['realized_cost'] for v in r['cases']]);name=algorithm+'_'+parameterization;costs[name]=values;per_case[f'{index}_{name}']=values.tolist();tables.append({'model_index':index,'algorithm':algorithm,'parameterization':parameterization,'mean_cost':float(values.mean()),'success':float(np.mean([v['success'] for v in r['cases']])),'mean_predicted_cost':float(np.mean([v['predicted_cost'] for v in r['cases']])),'mean_boundary_steps':float(np.mean([v['boundary_steps'] for v in r['cases']])),'mean_planning_seconds':float(np.mean([v['planning_seconds'] for v in r['cases']]))})
...
result={'scope':'previously examined scenarios; one training sample/seed; privileged-state open-loop controlled search, not published LeWM reproduction','cases_per_route':128,'search_scores_per_case':9000,'additional_verification_forwards_per_case':1,'bank_manifest_sha256':bank_hash,'zero_mean_cost':float(zero.mean()),'tables':tables,'contrasts':contrasts,'per_case_costs':per_case}
out=Path(a.output);out.parent.mkdir(parents=True,exist_ok=True);out.with_suffix('.json').write_text(json.dumps(result,indent=2)+'\n');lines=['# Budget-matched controlled planners','',result['scope'],'','| Model | Search | Action coordinates | Realized cost | Success | Predicted cost | Boundary steps |','|---|---|---|---:|---:|---:|---:|']
for v in tables:lines.append(f"| {v['model_index']} | {v['algorithm']} | {v['parameterization']} | {v['mean_cost']:.2f} | {v['success']:.1%} | {v['mean_predicted_cost']:.2f} | {v['mean_boundary_steps']:.2f} |")
