...
def main():
    ...
    p.add_argument('--task',choices=['reacher','pusht'],required=True);a=p.parse_args()
    ...
    for interface in interfaces:
        ...
        for i,row in enumerate(rows):
            ...
            np.testing.assert_allclose(s[:,i//2,:],x[:,:2],rtol=2e-5,atol=2e-5)
    ...
    r=dict(status='COMPLETE2_ENSEMBLE_CROSSFIT_GROUPS',task=a.task,groups=result,bindings=bindings,data_report_sha256=sha(data/'report.json'),baseline_report_sha256=sha(Path(a.baseline)/'report.json'),predictions_sha256=sha(out/'predictions.npz'),source_sha256=sha(__file__),ridge_source_sha256=sha(Path(__file__).with_name('crossfit_planner_regret.py')),scope='Consumed development banks; shared goal folds, fixed ridge1 and feature set. Three same-architecture pools share initialization. Both searches18000 actor candidates plus4 extra member action scores/encodings and3-model training; no equal-total-budget or independent confirmation claim. No search or method selection based on outcomes.')
