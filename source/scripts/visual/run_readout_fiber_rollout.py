...
def main():
    ...
    with torch.inference_mode():
        for slot in (2,3,4):
            ...
            assert fac['status']=='COMPLETE1536_FIRST_FEEDBACK_FIBER_CHECKS' and fac['constrained_valid']==1536 and fac['report_sha256']==sha(fd/'report.json') and fac['source_sha256']==sha(Path(__file__).with_name('accept_readout_fiber_rollout_inputs.py'))
            assert fr['protocol_sha256']==protocol_sha and fr['source_sha256']==sha(Path(__file__).with_name('prepare_readout_fiber_rollout.py')) and fr['projection_source_sha256']==sha(Path(__file__).with_name('readout_fiber.py'))
            ...
            values={path:[[] for _ in refs] for path in ('free','fiber','reset')};goals=[]
            for i,h in enumerate(histories):
                ...
                for k,examples in enumerate(refs):
                    ...
                    correction=fz['constrained'][j];err=abs((physical(correction)-physical(fz['predicted'][j]))/head['target_scale']);assert np.max(err)<=1e-6;readout_checks+=1
                    replacements={'fiber':torch.as_tensor(correction,device='cuda'),'reset':torch.as_tensor(fz['observed'][j],device='cuda')}
                    for path in values:
                        ...
                        else:np.testing.assert_array_equal(tokens[0],fz['constrained' if path=='fiber' else 'observed'][j])
    ...
    assert anchor_checks==readout_checks==1536 and goal_checks==384 and protocol_sha==sha(a.protocol)
    sources={p:sha(Path(__file__).with_name(p)) for p in ('run_readout_fiber_rollout.py','image_planner_cost.py','factorial_model.py','evaluation_precision.py','adaptation_freeze.py','extract_pose_selected_endpoints.py','lewm_adapter.py','accept_task_coordinate_horizon.py','accept_readout_fiber_rollout_inputs.py','prepare_readout_fiber_rollout.py','readout_fiber.py')}
    report=dict(status='FIBER_ROLLOUT_REQUIRES_NUMPY_ACCEPTANCE',index=a.index,rows=rows,reference_bindings=reference_bindings,anchor_checks=anchor_checks,readout_checks=readout_checks,goal_checks=goal_checks,input_bindings=input_bindings,plan_sha256=sha(a.plan),protocol_sha256=protocol_sha,source_sha256=sha(__file__),sources=sources,elapsed_seconds=time.monotonic()-started,gpu=torch.cuda.get_device_name(),scope='Single feedback intervention at the first predicted block: free, readout-preserving fiber correction and full observed reset. All3 JEPA objectives,128goals and4fixed references. Exact complete free-trajectory anchors; no new training/search or later observed feedback.')
