"""Complete prespecified paired fresh-goal comparison; require all96 accepted routes."""
...
def main():
    ...
    assert plan['task_coordinate_evaluation']['status']=='READY_COMPLETE_FRESH_EVALUATION' and len(plan['routes'])==96 and len(plan['models'])==48
    assert sha(bank/'manifest.json')==plan['bank_manifest_sha256'] and len(bm['cases'])==128 and bm['seed_start']==1351001
    ...
    rng=np.random.default_rng(1355001);draws=rng.integers(0,128,size=(10000,128));contrasts=[];groups=[]
    ...
    for parity,recipe in [(0,'pose_encoded'),(1,'state')]:
        ...
        conditions=('original','clipped_latent','unit_latent','unit_decoded_teacher','unit_physical_labels') if parity==0 else ('original','clipped_state','unit_state')
        comparisons=[('unit_decoded_teacher','unit_latent',True),('unit_physical_labels','unit_decoded_teacher',True),('clipped_latent','original',False),('unit_latent','clipped_latent',False),('unit_latent','original',False),('unit_decoded_teacher','original',False),('unit_physical_labels','original',False)] if parity==0 else [('clipped_state','original',False),('unit_state','clipped_state',False),('unit_state','original',False)]
        for algorithm in ('random','cem'):
            ...
            for left,right,primary in comparisons:contrasts.append(contrast(ids,(left,algorithm),(right,algorithm),recipe,primary))
        for condition in conditions:contrasts.append(contrast(ids,(condition,'cem'),(condition,'random'),recipe,False))
    assert sum(c['primary_cost'] for c in contrasts)==4
    result=dict(status='COMPLETE96_FRESH_TASK_COORDINATE_ROUTES',rows=rows,groups=groups,contrasts=contrasts,goal_seeds=seeds,bindings=bindings,plan_sha256=sha(a.plan),source_sha256=sha(__file__),bootstrap=dict(seed=1355001,replicates=10000,unit='same goal indices shared across all six models and all contrasts',primary='Four JEPA teacher-minus-unit-latent and physical-label-minus-teacher cost contrasts across random and CEM, nominal98.75% marginal percentile intervals; approximate Bonferroni-style presentation, not finite-sample certification.',secondary='95% descriptive intervals are not simultaneous guarantees.'),scope='All prespecified final conditions retained. Uncertainty conditional on these fixed fits, over admitted fresh goals; no training-population or cross-task inference. Remote acceptance binds full numerical search and physical replay; raw candidate archives not rechecked by this summary. Imagined and real-image error diagnostics remain separate.')
    with Path(a.output).open('x') as f:json.dump(result,f,indent=2);f.write('\n')
    print('COMPLETE96',flush=True)
