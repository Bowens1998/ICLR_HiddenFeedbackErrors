...
def main():
    ...
    a=p.parse_args();d=Path(a.run);r=json.loads((d/'report.json').read_text());assert r['protocol_sha256']==sha(a.protocol) and r['source_sha256']==sha(Path(__file__).with_name('run_readout_fiber_feasibility.py')) and r['projection_source_sha256']==sha(Path(__file__).with_name('readout_fiber.py'))
    assert len(r['records'])==720;verified=[];max_region=0;max_readout=0
    for g in range(6):
