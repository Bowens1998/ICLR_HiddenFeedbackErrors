"""Check whether fixed expert trajectories support a competent true-state GRU predictor."""
...
def cache(data,split,mean,std,am,asd):
    folder=Path(data)/split;e=np.load(folder/'episodes.npz');starts=np.concatenate([int(o)+np.arange(int(n)-19) for o,n in zip(e['offsets'],e['lengths'])])
    state=np.load(folder/'state.npy',mmap_mode='r');act=np.load(folder/'action.npy',mmap_mode='r')
    s=torch.from_numpy(np.array(state[starts[:,None]+np.array([0,5,10,15])[None]])).float().cuda()
    actions=torch.from_numpy(np.array(act[starts[:,None]+np.arange(15)[None]])).float().cuda().reshape(-1,3,10)
...
def main():
    ...
    p.add_argument('--index',type=int,required=True);p.add_argument('--epochs',type=int,default=100);a=p.parse_args();lr=[5e-5,3e-4,1e-3][a.index]
    ...
    report={'index':a.index,'base_lr':lr,'seed':3072,'expected_epochs':a.epochs,'epochs':[],'normalization':an,'target_normalization':norm,'precision':precision,'data_manifest_sha256':reference['data_manifest_sha256'],
            ...
            'selection':'minimum validation prediction MSE per LR; choose LR by that value before task scoring; retain all LR results','parameters':sum(p.numel() for p in model.parameters()),'gpu':torch.cuda.get_device_name()}
    ...
    for epoch in range(a.epochs):
        ...
        for begin in range(0,len(s)-127,128):
            ...
            with torch.autocast('cuda',dtype=torch.bfloat16):pred=model.predict(s[ids,:3],model.action_encoder(actions[ids]));loss=.5*(pred-s[ids,1:]).square().mean()
        ...
        with torch.inference_mode():
            for begin in range(0,len(s),128):predictions.append(model.predict(s[begin:begin+128,:3],model.action_encoder(actions[begin:begin+128])).float())
        pred=torch.cat(predictions);val=float((pred-s[:,1:]).square().mean());assert np.isfinite(val)
        if val<best:
            best=val;report['best_epoch']=epoch+1;report['best_validation_mse']=val;torch.save(model.state_dict(),out/'best_weights.pt')
            np.savez_compressed(out/'validation_best_predictions.npz',predicted=pred.cpu().numpy(),target=s[:,1:].cpu().numpy())
        row={'epoch':epoch+1,'lr':rate,'train_half_prediction_mse':total/count,'validation_prediction_mse':val,'clipped_batches':clipped};report['epochs'].append(row);report['wall_seconds']=time.perf_counter()-started
